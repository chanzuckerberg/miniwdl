...
def decl_to_inputs_dict(values_env: Env.Bindings[Decl]) -> Dict[str, Any]:
    """
    Convert a ``WDL.Env.Bindings[WDL.Value.Base]`` to a dict which ``json.dumps`` to
    an inputs json templates

    @param values_env: Env.Bindings
    @return dict that represent the input for the workflow
    """
    namespace = ""
    ans = {}
    for item in values_env:
        j = _binding_value_to_inputs(item.value.type)
        ans[(namespace if not item.name.startswith("_") else "") + item.name] = j
    return ans


def _binding_value_to_inputs(value: Type.Base) -> Any:
    """Converts a Type.Base value to its corresponding json type in
    inputs json.

    @param value: instance of Type.Base
    @return: corresponding json type
    """
    if isinstance(value, Type.StructInstance):
        ans = {}
        for member_name, member_type in value.members.items():
            ans[member_name] = _binding_value_to_inputs(member_type)
        return ans
    elif isinstance(value, Type.Array):
        return [_binding_value_to_inputs(value.item_type)]
    elif isinstance(value, Type.Map):
        (key, val) = value.item_type
        return {str(key): str(val)}
    elif isinstance(value, Type.Pair):
        return {"left": str(value.left_type), "right": str(value.right_type)}
    else:
        assert isinstance(value, Type.Base), type(value)
        return str(value)
