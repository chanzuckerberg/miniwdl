...
class CallCache(AbstractContextManager):
    ...
    def get(
        ...
        try:
            ...
            self._logger.info(_("call cache miss", cached=file_path))
        except Exception as exn:
            self._logger.warning(
                _("call cache entry present, but unreadable", cached=file_path, error=str(exn))
            )
        if cache:
            self._logger.notice(_("call cache hit", cached=file_path))  # pyre-fixme
    ...
    def put(self, key: str, outputs: Env.Bindings[Value.Base]) -> None:
        ...
        if self._cfg["call_cache"].get_bool("put"):
            ...
            self._logger.info(_("call cache insert", cached=filename))
