...
$miniwdl inputs --format json $SOURCE_DIR/test_corpi/gatk-workflows/five-dollar-genome-analysis-pipeline/germline_single_sample_workflow.wdl > germline_single_sample_workflow.inputs.json
is "$?" "0" "five dollar inputs"
is "$(jq length germline_single_sample_workflow.inputs.json)" "28" "five dollar inputs count"
$miniwdl inputs --format json --optional $SOURCE_DIR/test_corpi/gatk-workflows/five-dollar-genome-analysis-pipeline/germline_single_sample_workflow.wdl > germline_single_sample_workflow.inputs.json
is "$?" "0" "five dollar inputs --optional"
is "$(jq length germline_single_sample_workflow.inputs.json)" "37" "five dollar inputs --optional count"

$miniwdl inputs --format json $SOURCE_DIR/test_corpi/ENCODE-DCC/atac-seq-pipeline/atac.wdl --no-quant-check > atac.inputs.json
is "$?" "0" "ATAC-seq inputs"
is "$(jq length atac.inputs.json)" "3" "ATAC-seq inputs count"
$miniwdl inputs --format json $SOURCE_DIR/test_corpi/ENCODE-DCC/atac-seq-pipeline/atac.wdl --no-quant-check --optional > atac.inputs.json
is "$?" "0" "ATAC-seq inputs --optional"
is "$(jq length atac.inputs.json)" "94" "ATAC-seq inputs --optional count"
